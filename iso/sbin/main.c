...
void setup_system(void) {
    ...
    (void)mkdir("/lib", 0755); (void)mkdir("/lib64", 0755); (void)mkdir("/usr/lib", 0755);
    (void)setenv("PATH", "/bin:/sbin:/usr/bin:/usr/apps", 1);
    (void)setenv("LD_LIBRARY_PATH", "/lib:/lib64:/usr/lib:/usr/lib/x86_64-linux-gnu", 1);
...
void run_shell(const char *username) {
    ...
    while (1) {
        ...
        if (strcmp(input, "help") == 0) {
            printf("Built-ins: help, wifi, JurkStore, sysinfo, whoami, clear, logout, reboot, shutdown, busybox, cd, exit\n");
            printf("  wifi       - Scan nearby Wi-Fi networks and connect (Open or WPA/WPA2/WPA3)\n");
        ...
        if (found) {
            ...
            if (pid == 0) {
                ...
                // If bin_path is a symlink pointing to busybox, fallback to busybox execution
                char link_target[256];
                ssize_t r = readlink(bin_path, link_target, sizeof(link_target) - 1);
                if (r > 0) {
                    link_target[r] = '\0';
                    if (strstr(link_target, "busybox") != NULL) {
                        execve("/bin/busybox", args, env);
                    }
                }
                perror(cmd);
